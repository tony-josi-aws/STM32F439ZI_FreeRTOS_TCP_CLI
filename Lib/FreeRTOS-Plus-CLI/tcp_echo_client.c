...
#define configTCP_ECHO_SERVER_ADDR			"192.168.0.149"
	...
	for( ;; )
	{
		...
		if( FreeRTOS_connect( xSocket, &xEchoServerAddress, sizeof( xEchoServerAddress ) ) == 0 )
		{
			...
			for( lLoopCount = 0; lLoopCount < lMaxLoopCount; lLoopCount++ )
			{
				...
				/* If there is enough space in the stream buffer, write the data directly to the stream
				buffer to use zero copy. */
				if( xAvlSpace > 64 )
				{
					pcTxString = pucTCPZeroCopyStrmBuffer;
				}
				else
				{
					pcTxString = pcTransmittedString;
				}

				/* Add in some unique text at the front of the string. */
				sprintf( pcTxString, "TxRx message number %u", ulTxCount );
				ulTxCount++;
				uxDataLength = strlen(pcTxString);

				/* Are we using TCP zero copy? If yes, set the buffer to NULL */
				if(pcTxString != pcTransmittedString)
				{
					pcTxString = NULL;
				}
				...
				/* Send the string to the socket. */
				lTransmitted = FreeRTOS_send(	xSocket,						/* The socket being sent to. */
												( void * ) pcTxString,	/* The data being sent. */
												uxDataLength,					/* The length of the data being sent. */
												0 );							/* No flags. */
